...
const createLesson = asyncHandler(async (req, res) => {
  ...
  if (!lessonOrder) {
    const last = await Lesson.findOne({ academicYear, type })
      .sort({ order: -1 })
      .select("order")
      .lean();
...
const deleteLesson = asyncHandler(async (req, res) => {
  ...
  if (!id || !id.match(/^[0-9a-fA-F]{24}$/)) {
    return res
      .status(400)
      .json({ success: false, message: "معرف الدرس غير صحيح" });
  }
  ...
  if (lesson.items?.length) {
    ...
    for (const item of lesson.items) {
      ...
      try {
        ...
        const primaryType = "image";
        const delResult = await cloudinary.uploader.destroy(pubId, {
...
const togglePublish = asyncHandler(async (req, res) => {
  ...
  return success(
    ...
    lesson.published ? "تم نشر الدرس" : "تم إخفاء الدرس",
  );
});
...
async function assertStudentCanAccessLesson(studentId, lesson) {
  const student = await User.findById(studentId)
    .select("academicYear studentType")
    .lean();
...
const getStreamInfo = asyncHandler(async (req, res) => {
  ...
  return success(res, {
    ...
    embedUrl: ytId
      ? `https://www.youtube.com/embed/${ytId}?enablejsapi=1&rel=0&modestbranding=1&showinfo=0&iv_load_policy=3&color=white&playsinline=1`
      : null,
...
const addItem = asyncHandler(async (req, res) => {
  ...
  return success(
    res,
    { item: lesson.items[lesson.items.length - 1], lesson },
